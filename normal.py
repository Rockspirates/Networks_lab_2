...
def bitstring_to_qr_matrix(bitstring):
    ...
    bitstring_data = ''.join(['1' if bit else '0' for bit in bitstring])
...
def save_audio_to_wav(audio, filename, samplerate=440):
    """
...
def decode_qr_image(qr_image):
    ...
    qr_image_bgr = cv2.cvtColor(qr_image, cv2.COLOR_GRAY2BGR)
    
    # Decode the QR code using pyzbar
    decoded_objects = decode(qr_image_bgr)
...
bitstring = np.array([1 for i in range(10000)])

bits = audio_to_bitstring("output.wav")
